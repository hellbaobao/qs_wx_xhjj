...
//            判断消息按钮显示状态
    ...
    public function main() {
        ...
        $this->assign('apptlearnArr', $this->getApptlearnArr());
    ...
    public function getApptlearnArr() {
        $activArr = M('ApptLearnInfo')->where('is_publish=1')->order('id desc')->limit(10)->select();
        for ($i = 0; $i < count($activArr); $i++) {
            $activArr[$i]['cat_name'] = "培训";
    ...
    public function getMainNum() {
        ...
        $return['appt_learn'] = M('ApptLearnInfo')->where('is_publish=1')->count();
        $visit = M('ApptVisitInfo')->field("sum(appt_num) as appt_sum")->select();
        $return['visit_num'] = $visit[0]['appt_sum'] == NULL ? 0 : $visit[0]['appt_sum'];

        $learn = M('ApptLearnOrder')->field("sum(appt_num) as appt_sum")->select();
