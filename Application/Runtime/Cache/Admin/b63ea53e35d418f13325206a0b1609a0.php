            ...
            <form method="get" action="/index.php/Admin/SellerIntegralGoods/showList" class="form-horizontal" id="search-form" style="margin-top: 20px;">
                <div class="form-group">
                    <div class="col-sm-3" style="min-width: 300px;">
                        ...
                        <button type="button" class="btn btn-success delSellerInfo" onclick="javascript:void(window.location.href = '/index.php/Admin/SellerIntegralGoods/showListById')" style="height: 34px;">
                            显示全部积分商品
                        </button>
                    </div>
        ...
        <div class="table_content">
            <table class="table">
                ...
                <?php if(!empty($data["infoList"])): if(is_array($data["infoList"])): foreach($data["infoList"] as $k=>$v): ?><tr class="tr">
                        ...
                        <td>
                            <div>
                                <button class="btn btn-success" onclick="showGoodsInfo(<?php echo ($v["id"]); ?>, '<?php echo ($v["seller_name"]); ?>');">
                                    查看详情
                                </button>
                                <button class="btn btn-default edit-btn editSellerInfo" onclick="javascript:void(window.location.href = '/index.php/Admin/GoodsExchangeRecord/showListById/goods_id/<?php echo ($v["id"]); ?>')">
                                    兑换记录
                                </button>
                                <?php if($v["status"] == 1): ?><button class="btn btn-danger delSellerInfo change-goods-status" onclick="changeGoodsStatus('<?php echo ($v["id"]); ?>-<?php echo ($v["status"]); ?>')">
                                        下架
                                    </button>
                                    <?php elseif($v["status"] == 2): ?>
                                    <button class="btn btn-success delSellerInfo change-goods-status" onclick="changeGoodsStatus('<?php echo ($v["id"]); ?>-<?php echo ($v["status"]); ?>')">
                                        上架
                                    </button><?php endif; ?>
                            </div>
                        </td>
                        </tr><?php endforeach; endif; endif; ?>
                <?php if(empty($data["infoList"])): ?><tr><td colspan="10">暂无数据</td></tr><?php endif; ?>
            </table>
